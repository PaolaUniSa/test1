...
model = keras.models.Sequential([
    ...
    qkeras.QDense(neuron_number1, kernel_quantizer="binary(alpha=1)", use_bias=False),

    # keras.layers.BatchNormalization(),
    QActivation("quantized_bits(bits=24, integer=12)"),
    QBatchNormalization(gamma_quantizer=quantized_po2(24, 2),
                        variance_quantizer=quantized_po2(24, 2),
                        beta_quantizer=quantized_po2(24, 2)
                        ),

    keras.layers.Reshape((9,1)),
    QConv1D(1, 3,kernel_quantizer="binary(alpha=1)", use_bias=False),
    # qkeras.QDense(neuron_number2, kernel_quantizer="quantized_bits(24,12)", use_bias=False),
    # keras.layers.Flatten(),
    ...
    # keras.layers.BatchNormalization(),
    # QActivation("quantized_tanh(4,3)"),
    QActivation("quantized_bits(bits=24, integer=12)"),
    QBatchNormalization(gamma_quantizer=quantized_po2(24, 2),
                        variance_quantizer=quantized_po2(24, 2),
                        beta_quantizer=quantized_po2(24, 2)
    ...
    qkeras.QDense(output_shape, kernel_quantizer="binary(alpha=1)", use_bias=False),
    QBatchNormalization(gamma_quantizer=quantized_po2(24,2),
      variance_quantizer=quantized_po2(24,2),
      beta_quantizer=quantized_po2(24,2)
    ),
...
checkpoint_path = "C:/Unisa/AI in pressure sensor/DATASET/Matlab_dataset_high_temperature/model_Pressure_Data_Drift_Class1_test1_bin_weights.h5"
...
np.savetxt("C:/Unisa/AI in pressure sensor/DATASET/Matlab_dataset_high_temperature/Dataset_rawData/error_predicted_Class1_test1.txt",error_predicted.numpy())
